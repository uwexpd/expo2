  ...
  task :div_courses => :environment do
	...
	quarters.each do |quarter|
		...
		workbook.workbook.add_worksheet(name: "#{quarter.title}") do |sheet|
		  ...
		  sheet.add_row ['Abbreviation','Number', 'Short title', 'Course Title','Credits','#Enrolled','Enrollment Capacity','Room Capacity', 'Instructor Name', 'Instructor Email', 'Has Prerequiste']
		  ...
		  div_courses.each do |c|
		  	...
		  	sheet.add_row [c.dept_abbrev, c.course_no, c.short_title, c.course_title, c.credit_minimum.to_i, c.current_enroll, c.l_e_enroll, c.room_cap, instructor_names, instructor_emails, course_prereq]
		  end
		end
	end
	# Save the workbook to a file
	workbook.serialize('outputs/div_courses.xlsx')
