...
=end  
  def self.authenticate(uwnetid, password = nil, require_identity = nil, fail_if_person_not_found: false)
    uwnetid = uwnetid.to_s.match(/^(\w+)(@.+)?$/).try(:[], 1) # strip out the '@uw.edu' if someone tries that
    return false if uwnetid.blank?

    u = find_by_identity(uwnetid, require_identity)
