ActiveAdmin.register OfferingDashboardItem, as: 'dashboard_items' do
	...
	controller do
	    def new
	      super do |format|
	        build_nested_resource if resource.dashboard_item.nil?
	      end
	    end

	    def build_nested_resource
	      resource.build_dashboard_item
	    end
	end
	...
	sidebar "Offering Settings" do
  	...
  	form do |f|
		  ...
		  f.inputs for: :dashboard_item do |item_form|
		  	...
		  	# input_html: { class:  "tinymce", rows: 8 }
		  ...
		  f.inputs do
		  	# f.fields_for :dashboard_item do |item_form|
			#   	item_form.input :title
			#   	item_form.input :content, as: :text, input_html: { rows: 5, style: 'width: 100%'}item_form.input :css_class, label: 'Style', input_html: { style: 'width: 50%'}, hint: "You can include multiple CSS classes here, separated by spaces."
		  	# end
