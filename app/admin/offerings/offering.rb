ActiveAdmin.register Offering do
  ...
  index do
    ...
    column ('Unit') {|offering| link_to(offering.unit.short_title, admin_unit_path(offering.unit)) if offering.unit }
  ...
  sidebar "More Settings", only: :show do
    ul class: 'link-list' do
      ...
      li do
        span link_to "<i class='mi'>person_add_alt_1</i> Invitation Codes (#{OfferingInvitationCode.where(offering_id: offering.id, application_for_offering_id: nil).size} of #{offering.invitation_codes.size} left)".html_safe, admin_offering_invitation_codes_path(offering)
      end
      li do
    ...
    tabs do
      ...
      if offering.uses_awards?
        tab 'Awards' do
          f.inputs 'Award Parameteres' do
            ...
            f.input :default_award_amount, label: 'Default amount', :input_html => { :style => "width:15%;", value: object.default_award_amount.to_i }, hint: 'The default amount of funding for each quarter. Staff can decrease or increase individual awards on a case-by-case basis later in the process.'
      ...
      tab 'Applicants' do
        ...
        hr
          ...
          f.input :non_student, label: 'Non-Student application (such as fund/grant for faculty)', as: :boolean, hint: "Check this box to allow non-students to create applications for this offering. This allows anyone in the world to create an account in EXPO and start an application."
      ...
      tab 'Reviews' do
        f.inputs 'Reviews' do
          ...
          f.input :allow_to_review_mentee, as: :boolean, hint: "Reviewers cannot review their mentees' applications unless checked."
          f.input :reviewer_past_application_access, label: 'Access transcript and pass applications', as: :boolean, hint: "Reviewers cannot access transcripts or pass applications unless checked."
      ...
      if offering.uses_proceedings?
        tab 'Proceedings' do
          f.inputs 'Online Proceedings' do
            hr
            f.input :virtual_proceeding, label: 'Enable the virtual proceeding process and displays'
            f.input :proceeding_public_display_at, label: 'Public can only view virtual proceeding at:'
            f.input :proceedings_welcome_text, input_html: { class: "tinymce", rows: 30 }
            f.input :proceedings_pdf_letterhead, label: 'Letterhead', hint: 'This letterhead is the version used in the public proceedings.'
          end
        end
      end
    end
