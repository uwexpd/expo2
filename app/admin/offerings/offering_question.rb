    ...
    tabs do
      tab 'Question Details' do
			  f.inputs do
				...
				f.input :ordering, label: 'Order', as: :select, collection: page.questions.size==0 ? [1] : (1..page.questions.collect(&:ordering).compact.max()+1).to_a, include_blank: false, hint: 'You can also rearrange questions when viewing the page details.'
